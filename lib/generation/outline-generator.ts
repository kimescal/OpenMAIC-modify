 ...
 * Current product mode keeps course pages as slide-only:
 * - quiz/interactive/pbl are forced to slide
 * - interactive/pbl config checks are retained for logging context
 */
...
//    );
  ...
  if (outline.type !== 'slide') {
    log.info(`Forcing outline "${outline.title}" (${outline.type}) to slide-only mode`);
    return { ...outline, type: 'slide' };
  }
  return outline;
}
